...
def computeMatrix(states, lambda_value):
    """
...
def append_results(lambda_value, steady_state):
    ...
    with open(output, 'a') as f:
        for index, p in enumerate(steady_state[1:]):
            ...
            if p < 0:
                p = 0
            f.write('{},{},{}\n'.format(val, p, lambda_value)) 

def init_file():
    output = './model/data/output.csv'
    f = open(output, 'w+')
    f.write('state,prob,rate\n')
    f.close()
...
if __name__ == "__main__":
    ...
    for lambda_value in range(0, 1000, 10):
        # compute lambda
        if lambda_value == 0:
            lambda_value = 1
        l = lambda_value / 1000
        ...
        transition_matrix = computeMatrix(states, l)
        Q = np.ones((N, N + 1))
        ...
        tot = np.sum(steady_state_matrix[0])
        print('[ STEP ] {}, {}, {}'.format(l, np.all(error_matrix), tot))

        append_results(l, steady_state_matrix[0])
